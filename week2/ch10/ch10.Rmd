...
- For medical studies, it can be very expensive
...
- a **sample** is a number calculated from the **sample** (usually to estimate the parameter)
...
------|-----------|-----------
Mean  | $\bar{x}$ | $\mu$ (pronounced "mew")
Standard Deviation | $s$ | $\sigma$
Correlation | $r$ | $\rho$ (pronounced "row")
...
There are two alternative sampling methods we'll mention:
...
- Instead, we divide the forest into grids of 100 trees
- We then select 20 of these grids
...
Systematic sampling is cheaper than other methods, but requires very large samples to be representative
...
There are several bad, but common, sampling designs which don't give representative results.
...
## Bad Sampling Frames
We can get a **bad sampling frame** if we miss large groups in our population.  Common groups that are missed are:
...
- The questions can be leading or steer respondents in certain directions ("Should be improve", "Should be cut funding", etc).
