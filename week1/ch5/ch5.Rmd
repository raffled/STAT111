---
title: 'Chapter 5'
subtitle: 'The Standard Deviation as  Rule and the Normal Model'
author: "D. Raffle"
date: "5/20/2015"
output:
...
- A *symmetric distribution* is one that is "mirrored" around the center
- A *unimodal distribution* has one peak
- In a symmetric distribution, the *mean* and the *median* are the same
- If the data is *skewed*, we can often *re-express* it so that it is symmetric

In this chapter, we will concentrate on unimodal, symmetric distributions

- The sample mean is the measure of center, it is the "balancing point" of the distribution.
- The sample standard deviation is the measure of spread, it tells us how far, on average, observations are from the mean
...
- $n$ is our *sample size* or number of observations

Generally, we will use software like StatCrunch to find these values for us

## The Data
For this chapter, we will compare the SAT and ACT scores of 10000 students who took both exams.  
```{r}
...
- $Z_{ACT} = \frac{ACT - \overline{ACT}}{s_{ACT}} = \frac{26 - 21}{5} = = \frac{6}{5} = 1.2$
...
- She did better on the SATs than the ACTs
...
Say some Ivy League university will only accept students who score at least a 1750 on the SATs.  We can look at our distribution of SAT scores in terms of how far above (or below) our students are by subtracting 480 from every score.
...
- $1 lb = 0.454kg$
- $s = 978.5 lbs = (978 \times 0.454) kg = 444.2 kg$
