...
par(mfrow = c(1, 2))
with(titanic, {
    tab <- table(Sex[Survived == 'dead'])
    lbls <- paste(names(tab), " (", round(100*tab/sum(tab), 1), "%)", sep = "")
    pie(tab, lbls, main = "Sex of Dead")
    tab <- table(Sex[Survived == 'alive'])
    lbls <- paste(names(tab), " (", round(100*tab/sum(tab), 1), "%)", sep = "")
    pie(tab, lbls, main = "Sex of Survivors")
  }
)
par(mfrow = c(1, 1))
```
...
- Notice that we don't really know the survival rate of women from this chart, only the distribution of gender within each survival category.
- I.e., If many more people died than survived, there may be more women who died than survived.
- If more people survived than died, there could be more male survivors than male dead.
- The only way to know for sure would be to look at the counts, or invert the chart and graph the percentages of alive and dead within each sex.
...
- The frequency of students with hazel eyes
- The relative frequency of students with brown eyes
- The percentage of students with blue eyes
